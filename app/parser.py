...
# ─────────────────────────────────────────
# Step 4 — LLM-based dynamic skill extraction
# Catches skills ANYWHERE in resume — Skills section,
# experience, projects. No hardcoded list.
# ─────────────────────────────────────────

def extract_skills_llm(resume_text):
    """
    Uses Groq LLM to dynamically extract ALL skills from resume text.
    No hardcoded list. A safety filter verifies every returned skill
    actually appears in the resume text, preventing hallucination.
    """
...
Output format: a JSON array of strings and nothing else. No markdown, no headings,
...
"""
    ...
    try:
        ...
        verified = [s for s in skills if s.lower() in resume_lower]
...
def extract_skills(text):
    """
    Hybrid flow:
    1. Try section-based extraction (fast, free, no API call)
    2. Always run LLM extraction on full text (catches everything)
    3. Merge both — LLM acts as the dynamic safety net
    Returns (skills_list, method_used)
    """
    section_skills = []
    method = "fallback"
